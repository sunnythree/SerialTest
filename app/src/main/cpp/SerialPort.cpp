...
JNIEXPORT jobject JNICALL native_open(JNIEnv *env, jobject thiz, jstring path,jint baudrate) {
    ...
    /* Check arguments */
    {
        ...
        if (speed == -1) {
            ...
            LOGE("jinwei:Invalid baudrate","");
            return NULL;
        }
    }

    LOGD("jinwei:init native Opening device!","");
    /* Opening device */
    {
        jboolean iscopy;
    ...
    if(fcntl(fd,F_SETFL,0) < 0){   //阻塞，即使前面在open串口设备时设置的是非阻塞的，这里设为阻塞后，以此为准
        LOGE("jinwei : set seriral block failed","");
    }
    /* Configure device */
    {
        struct termios cfg;
        if (tcgetattr(fd, &cfg)) {
            LOGE("jinwei:%","Configure device tcgetattr() failed 1");
            close(fd);
            return NULL;
        }
        ...
        if (tcsetattr(fd, TCSANOW, &cfg)) {
            LOGE("jinwei","Configure device tcsetattr() failed 2");
            close(fd);
            /* TODO: throw an exception */
            return NULL;
        }
...
static int registerNatives(JNIEnv* env) {
    const char* kClassName = "com/example/administrator/serialtest/SerialPort"; //指定要注册的类
